...
$page_title = "Factures Clients";

// Requête de base pour le tableau (pagination manuelle)
$sqlBase = "SELECT f.numero_facture, f.titre_facture, f.date_facture, f.montant_ttc, f.avance, f.reste, f.etat_facture, ct.nom_prenom_contact AS client
            FROM facture f
            JOIN contact ct ON f.contact_id = ct.code_contact
            WHERE ct.type_contact = 'CLIENT'
...
<?php else: foreach ($factures as $row): ?>
    <tr>
        <td class="td-bold"><?= htmlspecialchars($row['numero_facture']) ?></td>
        <td><?= htmlspecialchars($row['client']) ?></td>
        <td><?= date('d/m/Y', strtotime($row['date_facture'])) ?></td>
        <td><strong><?= number_format((float)$row['montant_ttc'], 0, ',', ' ') ?> F</strong></td>
        <td><?= number_format((float)$row['avance'], 0, ',', ' ') ?> F</td>
        <td><?= number_format((float)$row['reste'], 0, ',', ' ') ?> F</td>
        <td>
            <?php
            $etat = $row['etat_facture'];
            if ($etat === 'Payée' || $etat === 'Payer cash') {
                $badge = 'badge-success';
...
<div class="W">
    <div class="hdr">
        <div><h1>Factures Clients</h1><p>Suivi des factures clients</p></div>
        <div class="header-badge"><i class="bi bi-file-earmark-text"></i> <?= $total ?> factures</div>
    </div>

    <div class="report-card">
        <h3><i class="bi bi-list-ul me-2"></i> Liste des factures clients <span class="text-muted small"><?= $total ?> lignes</span></h3>
        <div class="table-wrapper">
            <table>
                <thead><tr><th>N°</th><th>Client</th><th>Date</th><th>Montant TTC</th><th>Avance</th><th>Reste</th><th>État</th></tr></thead>
