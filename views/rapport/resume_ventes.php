...
$page_title = "Résumé des Ventes";

// Statistiques
$totaux = $pdo->query("SELECT COALESCE(SUM(CAST(montant_commande AS DECIMAL(12,2))),0) AS ca, COUNT(*) AS nb FROM commande WHERE statut_id='012' AND etat_commande NOT IN ('En attente','Annulé')")->fetch(PDO::FETCH_ASSOC);
$ca_total = $totaux['ca'];
$nb_ventes = $totaux['nb'];

// Graphiques
$evolCA = $pdo->query("SELECT DATE_FORMAT(date_commande,'%Y-%m') AS mois, COALESCE(SUM(CAST(montant_commande AS DECIMAL(12,2))),0) AS ca FROM commande WHERE statut_id='012' AND etat_commande NOT IN ('En attente','Annulé') AND date_commande >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH) GROUP BY mois ORDER BY mois ASC")->fetchAll(PDO::FETCH_ASSOC);
$catVentes = $pdo->query("SELECT cat.titre_categorie, COALESCE(SUM(CAST(c.montant_commande AS DECIMAL(12,2))),0) AS ca FROM commande c LEFT JOIN produit p ON c.produit_id=p.code_produit LEFT JOIN categorie cat ON p.categorie_id=cat.code_categorie WHERE c.statut_id='012' AND c.etat_commande NOT IN ('En attente','Annulé') GROUP BY cat.code_categorie ORDER BY ca DESC")->fetchAll(PDO::FETCH_ASSOC);

// Requête de base pour le tableau (pagination manuelle)
$sqlBase = "SELECT c.numero_commande, c.date_commande, c.heure_commande, c.quantite_commande, c.montant_commande, c.etat_commande, c.unite_affichage,
...
// Compter le total
$countSql = "SELECT COUNT(*) FROM commande WHERE statut_id='012' AND etat_commande NOT IN ('En attente','Annulé')";
...
<head>
    ...
    <style>
        ...
        .btn-go { background: #2563eb; color: #fff; padding: 7px 16px; border-radius: 8px; font-size: 12px; font-weight: 700; border: none; cursor: pointer; display: inline-flex; align-items: center; gap: 5px; }
...
<div class="W">
    <div class="hdr">
        <div><h1>Résumé des Ventes</h1><p>Chiffre d'affaires et analyse</p></div>
        <div class="header-badge"><i class="bi bi-cash-stack"></i> CA : <?= fmt($ca_total) ?> F</div>
    </div>

    <div class="stats-mini">
        <div class="stat-mini"><div class="value"><?= fmt($ca_total) ?> F</div><div class="label">CA Total</div></div>
        <div class="stat-mini"><div class="value"><?= $nb_ventes ?></div><div class="label">Nombre Ventes</div></div>
        <div class="stat-mini"><div class="value"><?= fmt($nb_ventes ? $ca_total/$nb_ventes : 0) ?> F</div><div class="label">Panier Moyen</div></div>
    </div>
    ...
    <div class="report-card">
        ...
        <div class="table-wrapper">
            <table>
                <thead><tr><th>N°</th><th>Date</th><th>Client</th><th>Produit</th><th>Cat.</th><th>Qte</th><th>TTC</th><th>Boutique</th><th>État</th></tr></thead>
...
$(document).ready(function() {
    ...
    if (ctx1) {
        new Chart(ctx1, {
            type: 'line',
            data: {
                labels: <?= json_encode(array_column($evolCA, 'mois')) ?>,
                datasets: [{
                    label: 'CA (FCFA)',
                    data: <?= json_encode(array_map(fn($r) => floatval($r['ca']), $evolCA)) ?>,
                    borderColor: '#2563eb',
    ...
    if (ctx2) {
        new Chart(ctx2, {
            type: 'doughnut',
            data: {
                labels: <?= json_encode(array_column($catVentes, 'titre_categorie')) ?>,
                datasets: [{
