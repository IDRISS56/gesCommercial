<?php
// stock_disponible.php – Gestion du stock disponible (par produit, boutique et lot)
...
function getStockDisponible($pdo, $search, $boutique_filter, $page, $perPage = 20) {
    // 1. Récupérer les produits avec leur stock total par boutique
    $sql = "
        SELECT 
            p.code_produit,
            p.titre_produit,
    ...
    // 2. Pour chaque ligne, récupérer les lots associés au produit
    foreach ($stocks as &$row) {
        ...
        foreach ($lots as $lot) {
            ...
            // On met à jour le disponible pour le prochain lot (on utilise le reste)
            $disponible = $reste;
        }
    ...
    <?php else: ?>
        <?php foreach ($stocks as $s): ?>
            <tr>
                ...
                <td><strong class="text-success"><?= (int)$s['stock_disponible'] ?></strong></td>
                <td>
                    <?php if (!empty($s['lots'])): ?>
...
if (isset($_POST['ajax']) && $_POST['ajax'] == '1') {
    ...
    if ($page < 1) $page = 1;
    $result = getStockDisponible($pdo, $search, $boutique_filter, $page);
...
$page = (int)($_POST['page'] ?? 1);
if ($page < 1) $page = 1;
$initialData = getStockDisponible($pdo, $search, $boutique_filter, $page);
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<div class="W">
    ...
    <div class="hdr">
        <div class="hdr-l">
            ...
            <p>Visualisation du stock par produit, boutique et lot</p>
        </div>
        <div class="hdr-r">
            <div class="hdr-badge"><i class="bi bi-boxes"></i> <?= $initialData['total'] ?? 0 ?> lignes</div>
        </div>
    </div>
    ...
    <div class="data-table-wrap" id="tableWrapper">
        <div class="d-flex flex-wrap align-items-center justify-content-between p-3 border-bottom bg-light">
            <h5 class="mb-0 fw-bold" style="font-family:'Outfit',sans-serif;">Stock par produit et boutique</h5>
