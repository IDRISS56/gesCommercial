<?php
// alerte_stock.php – Liste des produits en alerte ou en dessous du seuil d'alerte
...
function getRuptureStock($pdo, $search, $boutique_filter, $page, $perPage = 20) {
    // Requête pour obtenir les produits avec leur stock disponible par boutique
    $sql = "
        SELECT 
            p.code_produit,
            p.titre_produit,
    ...
    // On filtre sur les produits dont le stock disponible est inférieur ou égal au seuil d'alerte
    $sql .= " AND (sb.quantite - sb.quantite_reservee) <= p.stock_alerte";
    $sql .= " ORDER BY (sb.quantite - sb.quantite_reservee) ASC, p.code_produit, b.nom_boutique";
    ...
    foreach ($stocks as &$row) {
        ...
        $disponible = (int)$row['stock_disponible'];
        $lotsDisponibles = [];
        foreach ($lots as $lot) {
            $unites = (int)$lot['unites_par_lot'];
            $nbLots = intdiv($disponible, $unites);
            $reste = $disponible % $unites;
            $lotsDisponibles[] = [
                'titre' => $lot['titre_lot'],
                'code' => $lot['code_lot_produit'],
                'unites' => $unites,
                'nb_lots' => $nbLots,
                'reste' => $reste
            ];
    ...
    <?php else: ?>
        <?php foreach ($stocks as $s): 
            $enRupture = (int)$s['stock_disponible'] <= 0;
            $estEnAlerte = (int)$s['stock_disponible'] > 0 && (int)$s['stock_disponible'] <= (int)$s['stock_alerte'];
        ?>
            <tr>
                ...
                <td>
                    <?php if ($enRupture): ?>
                        <span class="status-badge off"><span class="sdot"></span> Alerte (0)</span>
                    <?php elseif ($estEnAlerte): ?>
                        <span class="status-badge" style="background: var(--wrnl); color: #b45309;">
                            <span class="sdot" style="background:#b45309;"></span> Alerte (<?= (int)$s['stock_disponible'] ?>)
                        </span>
                    <?php endif; ?>
                </td>
                <td>
                    <span class="badge bg-secondary">Seuil : <?= (int)$s['stock_alerte'] ?></span>
                </td>
                <td>
                    <?php if (!empty($s['lots'])): ?>
                    ...
                    <?php else: ?>
                        <span class="text-muted">Aucun lot</span>
                    <?php endif; ?>
                </td>
            </tr>
        <?php endforeach; ?>
...
<div class="W">
    ...
    <div class="hdr">
        <div class="hdr-l">
            ...
            <p>Produits dont le stock disponible est inférieur ou égal au seuil d'alerte</p>
        </div>
        <div class="hdr-r">
            <div class="hdr-badge"><i class="bi bi-exclamation-triangle"></i> <?= $initialData['total'] ?? 0 ?> alertes</div>
        </div>
    </div>
    ...
    <div class="data-table-wrap" id="tableWrapper">
        <div class="d-flex flex-wrap align-items-center justify-content-between p-3 border-bottom bg-light">
            <h5 class="mb-0 fw-bold" style="font-family:'Outfit',sans-serif;">Produits en alerte</h5>
            <span class="text-muted small" id="totalCount"><?= $initialData['total'] ?> ligne(s) - Page <?= $initialData['page'] ?> / <?= max(1, $initialData['totalPages']) ?></span>
        </div>
    ...
    function rechercher(page) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#totalCount').text(data.total + ' ligne(s) - Page ' + data.page + ' / ' + Math.max(1, data.totalPages));
