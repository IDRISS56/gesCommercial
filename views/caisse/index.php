            ...
            <tr>
                ...
                <td class="text-end">
                    <div class="d-inline-flex gap-1">
                        <button class="act-btn v viewBtn" data-code="<?= e($c['code_caisse']) ?>" title="Voir"><i class="bi bi-eye"></i></button>
...
if (isset($_POST['ajax_view']) && $_POST['ajax_view'] == '1') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT c.*, u.nom_prenom AS utilisateur_nom 
                               FROM caisse c 
                               LEFT JOIN utilisateur u ON c.utilisateur_id = u.id 
                               WHERE c.code_caisse = ?");
        $stmt->execute([$code]);
        ...
        if ($c) {
            ...
            echo json_encode([
                ...
                'titre_caisse' => $c['titre_caisse'],
                'solde_virtuel' => number_format((float)$c['solde_virtuel'], 2, ',', ' '),
                'solde_physique' => number_format((float)$c['solde_physique'], 2, ',', ' '),
                'utilisateur_nom' => $c['utilisateur_nom'] ?? '—',
                'etat_caisse' => $c['etat_caisse']
            ]);
        } else {
            header('Content-Type: application/json');
...
========================================================= -->
    <div class="modal fade" id="viewModal" tabindex="-1" aria-labelledby="viewModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width:600px;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="viewModalLabel"><i class="bi bi-eye text-primary me-2"></i> Détails de la caisse</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fermer"></button>
                </div>
                <div class="modal-body">
...
========================================================= -->
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.viewBtn', function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.success) {
                            $('#viewModalLabel').text('Caisse ' + res.titre_caisse);
                            const fields = [
                                ['Code', res.code_caisse],
                                ['Titre', res.titre_caisse],
                                ['Solde virtuel', res.solde_virtuel],
                                ['Solde physique', res.solde_physique],
                                ['Utilisateur', res.utilisateur_nom],
                                ['État', res.etat_caisse]
                            ];
                            let html = '';
                            ...
                            alert('Erreur : ' + (res.message || 'Caisse non trouvée'));
                        }
                    },
                    error: function() {
            ...
            // --- Si édition via POST ---
            <?php if (isset($editCaisse) && $action === 'edit'): ?>
                $(function() {
                    $('#formAction').val('edit');
