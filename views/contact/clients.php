                ...
                try {
                    if ($action === 'add') {
                        ...
                        if ($stmt->fetchColumn() > 0) {
                            ...
                            $message = "Client « $nom » ajouté avec succès. Code : $code";
                            $messageType = 'success';
                        }
                    } elseif ($action === 'edit') {
...
function getTableContent($pdo, $search, $filtres, $page, $perPage = 20) {
    $sql = "SELECT * FROM contact WHERE type_contact = 'Client'";
    $params = [];
    ...
    <tr>
        ...
        <td class="text-end">
            <div class="d-inline-flex gap-1">
                <button class="act-btn e editBtn" data-code="<?= e($c['code_contact']) ?>" title="Modifier">
                    <i class="bi bi-pencil"></i>
                </button>
                <button class="act-btn d deleteBtn" data-code="<?= e($c['code_contact']) ?>" data-nom="<?= e($c['nom_prenom_contact']) ?>" title="Supprimer">
...
// Statistiques
$totalClients = $pdo->query("SELECT COUNT(*) FROM contact WHERE type_contact = 'Client'")->fetchColumn();
$actifs = $pdo->query("SELECT COUNT(*) FROM contact WHERE type_contact = 'Client' AND etat_contact = 'Actif'")->fetchColumn();
$inactifs = $pdo->query("SELECT COUNT(*) FROM contact WHERE type_contact = 'Client' AND etat_contact = 'Inactif'")->fetchColumn();
$particuliers = $pdo->query("SELECT COUNT(*) FROM contact WHERE type_contact = 'Client' AND statut_contact = 'Particulier'")->fetchColumn();
$societes = $pdo->query("SELECT COUNT(*) FROM contact WHERE type_contact = 'Client' AND statut_contact = 'Société'")->fetchColumn();
$associations = $pdo->query("SELECT COUNT(*) FROM contact WHERE type_contact = 'Client' AND statut_contact = 'Association'")->fetchColumn();
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<div class="W">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-end mb-4 gap-2">
        <div>
            <h1 class="h3 fw-bold mb-1"><i class="bi bi-people-fill text-primary me-2"></i>Gestion des clients</h1>
            <p class="text-muted small mb-0">Gérez votre portefeuille clients</p>
        </div>
        <div class="d-flex gap-2">
            <span class="badge rounded-pill bg-primary-subtle text-primary border border-primary-subtle px-3 py-2">
                <i class="bi bi-people"></i> <?= $totalClients ?> client(s)
            </span>
    ...
    <div class="row g-3 mb-4">
        <?php
        $stats = [
            ['primary', 'people-fill', 'Total clients', $totalClients, ''],
    ...
    <div class="data-table-wrap">
        <div class="d-flex flex-wrap align-items-center justify-content-between p-3 border-bottom bg-light">
            <h5 class="mb-0 fw-bold">Liste des clients</h5>
            <span class="text-muted small"><?= $initialData['total'] ?> client(s) - Page <?= $initialData['page'] ?> / <?= max(1, $initialData['totalPages']) ?></span>
        </div>
...
<!-- Modal formulaire -->
<div class="modal fade modal-chic" id="clientModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">
                    <i class="bi bi-person-plus-fill"></i>
                    <span id="modalTitleText">Nouveau client</span>
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form method="post" id="clientForm" style="display: flex; flex-direction: column; flex: 1; min-height: 0;">
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content" style="border-radius:16px;border:none;">
            <div class="modal-body text-center p-4">
                ...
                <p class="text-muted small mb-4">Êtes-vous sûr de vouloir supprimer le client <strong id="deleteNomClient" class="text-danger"></strong> ?<br>Cette action est irréversible.</p>
...
$(document).ready(function() {
    ...
    $(function() {
        ...
        $('#modalTitleText').text('Modifier le client');
        setTimeout(function() {
            clientModal.show();
        }, 100);
    });
    ...
    // Suppression
    let clientCodeToDelete = null;
    $(document).on('click', '.deleteBtn', function() {
        clientCodeToDelete = $(this).data('code');
        const nom = $(this).data('nom');
        $('#deleteNomClient').text(nom);
        $('#deleteCodeField').val(clientCodeToDelete);
        deleteModal.show();
    });
