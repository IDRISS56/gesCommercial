...
<select name="role" id="roleFilter" class="selectpicker" data-live-search="true" data-live-search-placeholder="Tous">
...
<select name="etat" id="etatFilter" class="selectpicker" data-live-search="true" data-live-search-placeholder="Tous">
<option value="">Tous</option>
...
<div class="input-group">
<span class="input-group-text"><i class="bi bi-key"></i></span>
<input type="password" class="form-control" id="mdp" name="mdp" placeholder="Mot de passe initial">
</div>
<div class="form-text">Laisser vide pour utiliser le login comme mot de passe.</div>
</div>
<div class="col-md-6">
...
// === FONCTION POUR RÉINITIALISER PROPREMENT LE SELECTPICKER BOUTIQUE ===
function reinitialiserSelectpickerBoutique(valeurSelectionnee) {
var $select = $('#boutique_id');
// 1. Détruire complètement le selectpicker existant
$select.selectpicker('destroy');

// 2. Reconstruire les options depuis zéro
var optionsHtml = '<option value="">=== Aucune ===</option>';
<?php foreach ($boutiques as $b): ?>
var isSelected = '<?= e($b['code_boutique']) ?>' === valeurSelectionnee ? 'selected' : '';
optionsHtml += '<option value="<?= e($b['code_boutique']) ?>" ' + isSelected + '><?= e($b['nom_boutique']) ?></option>';
<?php endforeach; ?>
...
// Initialisation des selectpicker au chargement
...
// Réinitialiser le selectpicker boutique proprement
reinitialiserSelectpickerBoutique('');
...
$('#id').prop('readonly', true);
// Le mot de passe ne s'affiche pas en édition
$('#mdpFieldWrapper').hide();
$('#mdp').val('');

// Forcer la sélection du rôle
var editRole = <?= json_encode($editUser['role'] ?? '') ?>;
$('#role option').prop('selected', false);
$('#role').val(editRole);
toggleBoutiqueRequired(editRole);

// Réinitialiser le selectpicker boutique avec la valeur de l'utilisateur
var boutiqueId = <?= json_encode($editUser['boutique_id'] ?? '') ?>;
reinitialiserSelectpickerBoutique(boutiqueId);
