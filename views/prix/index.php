...
// --- Traitement des actions POST (inchangé) ---
$message = '';
$messageType = '';
$action = $_POST['action'] ?? '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Vérification CSRF (sauf pour actions view/edit via formulaire caché)
    if (empty($csrf_token) || $csrf_token !== ($_SESSION['csrf_token'] ?? '')) {
        if (!isset($_POST['ajax']) && !isset($_POST['ajax_view'])) {
            ...
            <tr>
                ...
                <td class="text-end">
                    <div class="d-inline-flex gap-1">
                        <button class="act-btn v viewBtn" data-code="<?= e($row['code_prix']) ?>" title="Voir"><i class="bi bi-eye"></i></button>
                        <button class="act-btn e editBtn" data-code="<?= e($row['code_prix']) ?>" title="Modifier"><i class="bi bi-pencil"></i></button>
                        <button class="act-btn d deleteBtn" data-code="<?= e($row['code_prix']) ?>" data-nom="<?= e($row['code_prix']) ?>" title="Supprimer" data-bs-toggle="modal" data-bs-target="#deleteConfirmModal"><i class="bi bi-trash3"></i></button>
                    </div>
                </td>
            </tr>
        <?php endforeach; ?>
...
if (isset($_POST['ajax_view']) && $_POST['ajax_view'] == '1') {
    ...
    try {
        $stmt = $pdo->prepare("SELECT p.*, pr.titre_produit, b.nom_boutique, lp.titre_lot AS lot_libelle
                               FROM prix p
                               LEFT JOIN produit pr ON p.produit_id = pr.code_produit
                               LEFT JOIN boutique b ON p.boutique_id = b.code_boutique
                               LEFT JOIN lot_produit lp ON p.lot_id = lp.code_lot_produit
        ...
        if ($row) {
            $lotLabel = ($row['lot_id'] === null) ? 'Unité de base' : ($row['lot_libelle'] ?? 'Unité');
            header('Content-Type: application/json');
            echo json_encode([
                'success' => true,
                'code_prix' => $row['code_prix'],
                'titre_produit' => $row['titre_produit'] ?? $row['produit_id'],
                'nom_boutique' => $row['nom_boutique'] ?? 'Global',
                'lot_libelle' => $lotLabel,
                'titre_prix' => $row['titre_prix'],
                'quantite_min' => $row['quantite_min'],
                'quantite_max' => $row['quantite_max'] ?? 'Illimité',
                'prix_unitaire' => number_format((float)$row['prix_unitaire'], 2, ',', ' '),
                'etat_prix' => $row['etat_prix']
            ]);
        } else {
            header('Content-Type: application/json');
            echo json_encode(['success' => false, 'message' => 'Prix non trouvé']);
        }
    } catch (PDOException $e) {
        header('Content-Type: application/json');
...
<body>
    ...
    <div class="modal fade" id="viewModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width:600px;">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="viewModalLabel"><i class="bi bi-eye text-primary me-2"></i> Détails du prix</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
    ...
    <script>
        $(document).ready(function() {
            // Initialisation des selectpicker (uniquement sur les éléments qui ont la classe .selectpicker)
            // NOTE : #lot_id n'est PAS un selectpicker, c'est un <select> natif : on ne l'initialise
            // jamais avec bootstrap-select, ce qui évite le bug de désynchronisation entre produits.
            $('.selectpicker').selectpicker('destroy').selectpicker();

            // ---- PRÉCHARGEMENT DES LOTS EN JAVASCRIPT ----
            var allLots = <?= json_encode($lots) ?>;

            // Fonction pour filtrer et mettre à jour le select natif des lots (CORRIGÉE)
            function updateLotSelect(produitId, selectedLotId) {
                var lotSelect = $('#lot_id');
                // Vider complètement le select
                lotSelect.empty();
                // Ajouter l'option "Unité de base" (toujours présente)
                lotSelect.append('<option value="">Unité de base</option>');
                if (produitId) {
                    // Filtrer les lots correspondant au produit sélectionné (comparaison en chaîne)
                    var lotsFiltres = allLots.filter(function(lot) {
                        return String(lot.produit_id) === String(produitId);
                    });
                    // Ajouter chaque lot correspondant
                ...
                // Sélectionner la bonne valeur directement sur le select natif
                // (pas de selectpicker('refresh') ici : #lot_id n'est pas un selectpicker)
                lotSelect.val(selectedLotId || '');
            }

            // Événement sur le changement de produit : recharge la liste des lots à chaque fois
            ...
            $('#addBtn').on('click', function(e) {
                ...
                // Réinitialiser le select des lots avec "Unité de base"
            ...
            $(document).on('click', '.viewBtn', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.success) {
                            $('#viewModalLabel').text('Prix ' + res.code_prix);
                            const fields = [
                                ['Code', res.code_prix],
                                ['Produit', res.titre_produit],
                                ['Boutique', res.nom_boutique],
                                ['Lot/Unité', res.lot_libelle],
                                ['Type', res.titre_prix],
                                ['Quantité min', res.quantite_min],
                                ['Quantité max', res.quantite_max],
                                ['Prix unitaire', res.prix_unitaire + ' FCFA'],
                                ['État', res.etat_prix]
                            ];
                            let html = '';
                            ...
                            alert('Erreur : ' + (res.message || 'Prix non trouvé'));
                        }
                    },
                    error: function() {
                        alert('Erreur réseau.');
            ...
            <?php if (isset($editPrix) && $action === 'edit'): ?>
                $(function() {
                    $('#formAction').val('edit');
                    ...
                    // Mettre à jour le select des lots en fonction du produit et pré-sélectionner
